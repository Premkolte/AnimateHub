...
function Dashboard() {
  ...
  const filteredButtons = useMemo(() => {
    ...
    if (activeFilter !== "All") {
      buttons = buttons.filter(({ button }) =>
        button.toLowerCase().includes(activeFilter.toLowerCase())
      );
    }
  ...
  const handleFilterChange = useCallback((filter) => {
    ...
    setActiveTab(0); // Reset to first tab when filter changes
  }, []);

  const handleKeyDown = useCallback(
    (e) => {
      if (!debouncedSearchQuery.trim() || filteredButtons.length === 0) return;
      const currentIndex = filteredButtons.findIndex(
        ({ originalIndex }) => originalIndex === activeTab
      );
      if (e.key === "ArrowDown") {
        e.preventDefault();
        const nextIndex =
          currentIndex < filteredButtons.length - 1 ? currentIndex + 1 : 0;
        setActiveTab(filteredButtons[nextIndex].originalIndex);
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        const prevIndex =
          currentIndex > 0 ? currentIndex - 1 : filteredButtons.length - 1;
  ...
  useEffect(() => {
    if (filteredButtons.length > 0) {
      // If current activeTab is not in filtered results, set to first filtered result
      const isActiveTabInFiltered = filteredButtons.some(
        ({ originalIndex }) => originalIndex === activeTab
      );
      if (!isActiveTabInFiltered) {
        setActiveTab(filteredButtons[0].originalIndex);
      }
    }
  }, [filteredButtons, activeTab]);
          ...
          {(debouncedSearchQuery || activeFilter !== "All") && (
            <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 text-center">
              {filteredButtons.length > 0
                ? `Found ${filteredButtons.length} component${
                    filteredButtons.length === 1 ? "" : "s"
                  }`
                : "No components found"}
              {filteredButtons.length > 0 && (
